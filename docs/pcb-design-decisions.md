...
**Décisions tranchées ensuite par l'utilisateur (2026-08-23, même journée) :**

- **Pads thermiques U2/U3 : volontairement isolés.** L'isolation faite sur le PCB (pads sur le net GND mais physiquement hors de la zone de masse, d'où les 2 `unconnected` en DRC) est intentionnelle — isolation thermique du SHT41, et Sensirion déconseille explicitement de souder son pad central. Le **schéma** a été aligné en conséquence : la liaison à GND de U2 pin5 et U3 pin7 est remplacée par un **drapeau no-connect**. Sans ça, la prochaine synchronisation « Update PCB from Schematic » aurait silencieusement re-relié les deux pads à la masse.
- **R14 (100kΩ) ajoutée sur ADDR du BH1750.** ROHM prescrit une résistance (≈100kΩ) plutôt qu'une liaison directe lorsqu'aucune impulsion basse de ≥1µs n'est appliquée sur DVI au démarrage — exactement notre cas, DVI étant câblée en dur sur SENSOR_3V3 (elle monte *avec* VCC, aucune impulsion possible). Et comme SENSOR_3V3 est maintenant commuté par Q3, cette séquence se rejoue **à chaque cycle de mesure** au lieu d'une seule fois au boot. ADDR reste lue à l'état bas, donc l'adresse I2C demeure 0x23. *Contrainte de placement notée au passage : impossible de faire descendre ADDR verticalement — la broche GND de U3 est sur le même axe X, un fil droit les aurait court-circuitées. D'où le coude vers la gauche avant R14.*
- **Discipline firmware → issue GitHub [#19](https://github.com/ludodefgh/PlantSensor/issues/19).** Mettre P1.10/P1.11 en entrée déconnectée quand SENSOR_EN est bas, sous peine de fantôme-alimenter les capteurs éteints (~0.7mA par ligne via R1/R2 et les diodes ESD) et de sortir du maximum absolu des deux capteurs. Le matériel est correct : c'est une contrainte créée par la nouvelle architecture d'alimentation, à porter côté firmware.

**Correction à faire dans le brief :** `docs/host-pcb-design-brief.md` §4 spécifie toujours « Source → CR2032+ » pour Q1, soit l'orientation erronée décrite plus haut. C'est la raison pour laquelle le premier audit l'a validée. À corriger avant qu'une future session ne reparte de ce document (commenté aussi sur l'issue [#17](https://github.com/ludodefgh/PlantSensor/issues/17)).
